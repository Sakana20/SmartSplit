...
所有自动分句器返回前都会执行短分句合并后处理：非保护分句去掉标点、空白和分隔符后的折算长度小于 2 个汉字时，优先合并到同段落前一个非保护分句；前一个不可用时再尝试合并到同段落后一个非保护分句。合并不受 10 字硬上限限制，不跨段落，不吸收 `protected` 分句，并从清理后的稿件连续区间重建 `text` 与字符范围。
