...
- SRT 默认将首条有效字幕对齐媒体起点、末条有效字幕精确到毫秒对齐原始媒体第一个视频流的呈现结尾；纯音频输入对齐第一个音频流结尾。渲染还会填充不超过 20 帧（精确计算 667ms，约 670ms）的短间隙，并尽量把不足 200ms 的字幕延长到最短时长；修正只作用于 SRT 副本并写入 `subtitle_render_report.json`。
